...
URLS.append(Post(page_type=1,  #이거 안됨
                 ...
                 post_xpath='/html/body/div[1]/div[6]/form/div[2]/div[3]/ul[2]/li[%d]/h4/a',
                 more_page_xpath='//*[@id="listPlusAdd"]',
                 special=['/html/body/div[1]/div[6]/form/div[2]/div[3]/ul[1]/li[%d]/h4/a']))
...
def crawl_post_page(post_start, post_limit, post_xpath, keyword):
    global driver
...
def crawl_post_more(post_xpath, keyword):
    for tmp in range(1, 999):
        try:
            driver.find_element_by_xpath(post_xpath % (tmp))
        except selenium.common.exceptions.NoSuchElementException as e:
            post_limit = tmp - 1
            break

    for post_num in range(1, post_limit + 1):
        title = driver.find_element_by_xpath(post_xpath % (post_num)).text
        title = string_escaping_for_file(title)
        if title.find(keyword) != -1:  # 키워드 기반 필터링
            print(title)
    ...
    for page_num in range(URL.page_start, PAGE_LIMIT + 1):
        ...
        if "//*" in URL.next_page:
            if URL.page_offset_start <= page_num:
                driver.find_element_by_xpath(URL.next_page % (next_page_num+URL.page_offset)).click()
            else:
                driver.find_element_by_xpath(URL.next_page % (next_page_num)).click()
        else:
            driver.get(URL.get_page_url(page_num))
        time.sleep(WAIT_SEC)
        if URL.special:
            crawl_post_page(int(URL.special[1]), int(URL.special[2]), URL.special[0], '창업')
        crawl_post_page(URL.post_start, URL.post_limit, URL.post_xpath, '창업')
...
def type_more_post(URL: Post):
    ...
    while True:
        try:
            ...
            break
    if URL.special:
        crawl_post_more(URL.special[0], '창업')
    crawl_post_more(URL.post_xpath, '창업')
